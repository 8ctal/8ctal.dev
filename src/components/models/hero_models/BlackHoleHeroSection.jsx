// Ported from ref_components/black_hole_hero_section/black_hole_hero_section.tsx
// (21st.dev community component) — ES2020+ raymarched black hole on plain
// WebGL2/WebGL1, no react-three-fiber/drei/three.js involved. Replaces the
// old WebGPU pipeline (see BlackHoleCanvas.jsx/black_hole/) as the Hero's
// centerpiece per the redesign brief: this renders everywhere WebGL runs
// (desktop *and* mobile, where the WebGPU version never worked at all,
// since WebGPU support skews younger and mobile Chrome's WebGPU rollout is
// still partial), at the cost of the old version's drag-to-orbit
// interactivity — the camera here holds still and only the gas moves (see
// `orbitSpeed`, default 0, in the doc comment below). That trade was made
// deliberately in the redesign brief, not an oversight.
//
// Porting notes (TypeScript → this project's plain JSX, no other tooling
// changes):
// - Dropped `"use client"` (a Next.js/RSC directive; meaningless in a Vite SPA).
// - Dropped `import * as React from "react"` — this repo uses the automatic
//   JSX runtime (no other component here imports a default `React` either).
// - The `BlackHoleHeroSectionProps` TS interface doesn't exist in JS; its
//   per-prop documentation is kept just below as plain comments instead.
// - Every other TS-only construct (type annotations, `as` casts, the `!`
//   non-null assertion operator, the `Prog`/`Target` type aliases) was
//   mechanically stripped — the runtime logic, including every explanatory
//   comment throughout the file, is otherwise unchanged from the reference.
...
// Props for {@link BlackHoleHeroSection}. Every field is optional; see
// the inline comments in the original reference component
// (ref_components/black_hole_hero_section/black_hole_hero_section.tsx) for
// the full explanation of each one — kept here only as a name index since
// plain JS has nowhere to hang the per-prop doc comments a TS interface
// would.
// `distance` — Camera distance from the hole, in horizon radii.
// `elevation` — How far the camera sits above the disc, in degrees. Near 0 the disc is
...
// `azimuth` — Where the camera sits around the hole, in degrees.
// `orbitSpeed` — Degrees the camera swings around the hole each second. 0 by default, and
...
// `roll` — Turns the picture about the line of sight, in degrees, so the disc runs on a diagonal.
// `fov` — Vertical field of view, in degrees.
// `diskInner` — Inner edge of the disc, in horizon radii. 3 is the innermost stable orbit; below it gas falls in.
// `diskOuter` — Outer edge of the disc.
// `diskThickness` — Half-thickness of the disc at its inner edge. It flares outward from there.
// `diskDensity` — How much gas there is. Above 1 the near side starts to hide the far side.
// `brightness` — Overall brightness of the gas before tone mapping.
// `spinSpeed` — Turns of the inner rim per second. The rest of the disc follows Kepler, so
...
// `grain` — Size of the turbulence in the gas. Higher is finer.
// `doppler` — Relativistic beaming, 0 to 1. The gas at the inner rim runs at 0.4c, so
...
// `hotColor` — Brightest gas, at the inner rim.
// `midColor` — Mid-disc.
// `coolColor` — Coolest gas, at the outer edge.
// `starBrightness` — Background stars, 0 to 2. Off by default. They are lensed along with
...
// `glow` — Bloom, 0 to 2.
// `exposure` — Exposure into the tone curve.
// `vignette` — Corner darkening, 0 to 1.
// `steps` — Steps each ray may take. This is the one dial that costs real time. 300
// is clean; 180 will do on a phone; below 140 the disc starts to band.
// `resolution` — Render scale, 0.5 to 1. Drop it before you drop `steps`.
// `maxDpr` — Cap on device pixel ratio.
// `focus` — Where the hole sits in the frame, as fractions of width and height, top
...
// hero clear.
// `scrim` — Lays a veil over one edge so text can sit there and still be read.
// `scrimStrength` — How dark that edge gets, 0 to 1.
